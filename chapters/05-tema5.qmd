...
#### Gráfico de los residuos frente al orden de las observaciones {.unnumbered} 
...
#### Gráfico de los residuos frente a las variables explicativas {.unnumbered} 
...
### ¿Qué debemos buscar en un gráfico? {.unnumbered} 
...
$$
\hat e_i^2=
\alpha_0+\alpha_1LIQ_i+
\alpha_2LEV_i+
\alpha_3LIQ_i^2+
\alpha_4LEV_i^2+
\alpha_5LIQ_iLEV_i+v_i.
$$
...
### Test de Breusch-Pagan {.unnumbered} 
...
\beta_0\frac{1}{LIQ_i}
+
\beta_1\frac{LIQ_i}{LIQ_i}
+
\beta_2\frac{LEV_i}{LIQ_i}
+
\beta_3\frac{EMP_i}{LIQ_i}
+
\frac{u_i}{LIQ_i}.
$$
...
```{r}
datos$ROA_t <- datos$ROA / datos$LIQ
datos$inv_LIQ <- 1 / datos$LIQ
datos$LIQ_t <- datos$LIQ / datos$LIQ
datos$LEV_t <- datos$LEV / datos$LIQ
datos$EMP_t <- datos$EMP / datos$LIQ
```
...
$$
\frac{ROA_i}{LIQ_i}=\beta_0\frac{1}{LIQ_i}+\beta_1+\beta_2\frac{ENDEUDAMIENTO_i}{LIQ_i}+
\beta_3\frac{EMP_i}{LIQ_i}+u_i^*.
$$
...
1. La inmobiliaria InmoGranada S.A. desea realizar un estudio sobre el valor de la vivienda en la ciudad de Granada, con el objetivo de orientar futuras inversiones y decisiones comerciales. Para ello, ha recogido la siguiente información de las transacciones del último mes:
el precio de venta en miles de euros ($precio_i$), el número de
habitaciónes ($nhab_i$) y una variable que toma el valor 1 si la vivienda tiene una antigüedad de más de 10 años ($ant_i$), especificando el siguiente modelo:

$$precio_i=\beta_1+\beta_2nhab_i+\beta_3ant_i+u_i$$ A partir de la siguiente información:

$$(X'X)^{-1}=\left(
  \begin{array}{ccc}
   0.6657&-0.1424&-0.2994\\
  -0.1424& 0.0610& -0.0145\\
  - 0.2994& -0.0145& 0.4796\\
  \end{array}
\right); X'y=\left(
  \begin{array}{c}
    2057.88\\
    ...
    1307.88\\
  \end{array}
\right);\sum_{i=1}^{n=10} precio_i^2=455.396,079$$ Se pide:

a. Obtener los estimadores de los parámetros del modelo e interpretarlos.

b. Realice el contraste de significación global del modelo y construya la Tabla Anova.
...
library(readxl)
datos <- read_excel("viviendas_granada.xlsx")
norte <- ifelse(datos$zona == "Norte", 1, 0)
este <- ifelse(datos$zona == "Este", 1, 0)
oeste <- ifelse(datos$zona == "Oeste", 1, 0)
sur <- ifelse(datos$zona == "Sur", 1, 0)
data_df <- data.frame(
  superficie = datos$superficie,
  numhab = datos$num_hab,
  ant = datos$ant,
  norte = norte,
  este = este,
  oeste = oeste,
  precio = datos$precio/1000
)
modelo1=lm(precio~ log(superficie)+numhab+ant, data=data_df)
summary(modelo1)
modelo2=lm(precio~ superficie+numhab+ant, data=data_df)
summary(modelo2)
```
getwd()
$$\hat{precio}\_i=23.5156+1.8161sup_i-4.3921nhab_i-24.8294ant_i;
R^2=0.8441$$

c. Compare este modelo con el anterior en terminos de bondad del ajuste.

d. Sospechan que la inclusión de la variable superficie en el modelo ha generado que la **varianza de las perturbaciones del modelo no sea constante**. Para analizarlo, ordenan los datos de menor a mayor en función de la variable *superficie*, eliminan 18  observaciones centrales y realizan una regresión para cada una de  las submuestras resultantes, obteniendo los siguientes resultados:

    $$i=1-16: \hat{precio}=41.1831+1.1278superficie-0.3728nhab-4.765ant; R^2=0.4266;SCT=6193.02$$
    $$i=35-50: \hat{precio}=129.8725+1.2084superficie-9.99nhab-36.8128ant; R^2=0.6237;SCT=16613$$
    Realice el contraste necesario y obtenga las conclusiones al 95% de
    confianza.
